...
final class OverlayWindowController: NSWindowController {
    ...
    init(hostApp: SwiftPermisoHostApp, panel: SwiftPermisoPanel, onBack: @escaping () -> Void) {
    ...
    func present(from sourceFrameInScreen: CGRect?, settingsFrame: CGRect, visibleFrame: CGRect) {
        ...
        launchTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true) { [weak self] _ in
    ...
    private func springProgress(at elapsed: TimeInterval) -> CGFloat {
        let omega = (2 * Double.pi) / launchAnimationDuration
        let t = max(0, elapsed)
        return min(max(1 - exp(-omega * t) * (1 + (omega * t)), 0), 1)
    }
    ...
    private func anchoredOrigin(for settingsFrame: CGRect, visibleFrame: CGRect) -> NSPoint {
        ...
        let preferredY = settingsFrame.minY + 30
...
private final class OverlayContentView: NSView {
    ...
    private func setup(hostApp: SwiftPermisoHostApp, panel: SwiftPermisoPanel) {
        ...
        NSLayoutConstraint.activate([
            ...
            arrowView.leadingAnchor.constraint(equalTo: materialView.leadingAnchor, constant: 86),
